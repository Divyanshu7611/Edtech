...
function MyCourse() {
    ...
    useEffect(() => {
        dispatch(fetchInstructorCourse()).then(() => setLoading(false));
        console.log("redux data frontend", courses);
    }, [dispatch]);
            ...
            <div className='border border-[#161D29] rounded-lg mx-auto flex flex-col py-1 mt-5'>
                ...
                {courses && courses.length > 0 ? (
                    courses.map((course) => (
                        <div 
                            key={course.id} 
                            className="flex justify-between items-center py-2 px-4 border-b border-[#161D29] text-[#F1F2FF]"
                        >
                            <span className='w-2/4'>
                            <CourseCardDashboard key={course.id} title={course.courseName} description={course.courseDescription} thumbnail={course.thumbnail} status={'DRAFT'} createdAt={course.createdAt}/>
                            </span>
                            <span>{course.duration || 'N/A'}</span>
                            <span>${course.price || 'Free'}</span>
                            <div className="flex items-center gap-4">
                                <FaEdit className="text-yellow-400 cursor-pointer hover:text-yellow-300" title="Edit" />
                                <FaTrash className="text-red-500 cursor-pointer hover:text-red-400" title="Delete" />
                            </div>
                        </div>
                    ))
                ) : (
