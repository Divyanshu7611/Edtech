...
export default function Navbar2() {
  ...
  //         navbar.style.paddingBlock = "0.25rem";
  ...
  return (
    <header
      className="flex flex-wrap sm:justify-start sm:flex-nowrap z-50 w-full text-sm sm:py-0 fixed top-0 transition-all bg-[white]"
      id="navbar"
    >
      <nav
        className="relative max-w-7xl w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8 py-4"
        aria-label="Global"
      >
        ...
        >
          <div className="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:justify-center sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:pl-7">
            {navLinks.map((link) => (
              <a
                className="font-semibold sm:py-2 hover:text-[--primary] navlinks text-sm raleway text-[--secondary]"
                href={link.link}
                aria-current="page"
                key={link.link}
              >
